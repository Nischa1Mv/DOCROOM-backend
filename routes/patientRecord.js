...
router.get("/", verifyToken, async (req, res) => {
    try {
        ...
        if (!patientRecords || patientRecords.length === 0) {
            return res.status(200).json({ patientRecords: [], recordDetails: [], doctorName: doctor.name });
        }
        ...
        res.status(200).json({ patientRecords: populatedRecords, recordDetails, doctorName: doctor.name });
    } catch (error) {
